...
function save() {
  order_pokedex();
  fs.writeFile(
    `${extensionPath}/dist/data/savefile.json`,
...
function order_pokedex() {
  pokemon_boxes.sort((a, b) => a.id - b.id);
}
...
function init_tallgrass() {
  ...
  function _catchAttempt(pokeball: string) {
    ...
    for (var i = 0; i < rates.length; i++) {
      if (rates[i].name == pokeball) rate = rates[i].rate;
    }
    ...
    if (catch_num <= pokemon_num) {
      ...
      if (fleeCheck < pokedex[shown_pokemon].exp) {
        vscode.window.showErrorMessage(
          "Sad day, " + pokedex[shown_pokemon].name + " ran away..."
        );
        vscode.window.activeTextEditor?.setDecorations(newDecoration, []);
        shown_pokemon = -1;
      } else {
        vscode.window
          .showWarningMessage(
            "Oh no, the " + pokedex[shown_pokemon].name + " broke free!",
            {},
            ...["Try Again"]
          )
  ...
  function _catch() {
    ...
    vscode.window.showQuickPick(catch_options).then(function (choice) {
      if (choice) {
        for (var i = 0; i < items.length; i++) {
          if (items[i].name == choice) items[i].amount--;
        }
        save();
        switch (choice) {
          case "Pokeball":
          ...
          case "Razzberry":
            ...
            vscode.window
              .showInformationMessage(
                "You fed the Razzberry to " + pokedex[shown_pokemon].name + "!",
                {},
                ...["Next"]
              )
              .then(function (chosen) {
    ...
    if (n == 1 || n == 2) {
      ...
      newDecoration = vscode.window.createTextEditorDecorationType({
        ...
        gutterIconPath: vscode.Uri.parse(
          extensionPath + "/imgs/sprites/" + (shown_pokemon + 1) + ".png"
        ),
      });
      if (vscode.window.activeTextEditor?.selection.active.line) {
        var opts = {
          range: new vscode.Range(
            vscode.window.activeTextEditor.selection.active.line + 1 - 1,
            0,
            vscode.window.activeTextEditor.selection.active.line + 1 - 1,
            0
          ),
      ...
      var item_drops = [];
      item_drops.push(
        item_drop_possibilities[
          Math.floor(Math.random() * item_drop_possibilities.length)
        ]
      );
      item_drops.push(
        item_drop_possibilities[
